...
#include "TUMHFT_payload_xport.h"
...
T_DjiReturnCode DjiTest_XPortStartService_TUMHFT(void)
{
    ...
    // Set Gimbal Max. Yaw Angle
    if (aircraftInfoBaseInfo.mountPosition == DJI_MOUNT_POSITION_PAYLOAD_PORT_NO1) {
        limitAngle.upperLimit = 3000;
        limitAngle.lowerLimit = -3000;
    ...
    if (djiStat != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        ...
        USER_LOG_ERROR("Set gimbal max. yaw angle successful. -tumhft");
    }
    
    // DEFAULT: DJI_GIMBAL_MODE_FREE
    djiStat = DjiXPort_SetGimbalModeSync(DJI_GIMBAL_MODE_FREE);
    USER_LOG_ERROR("X-PORT GIMBAL MODE DJI_GIMBAL_MODE_FREE. -tumhft");
    if (djiStat != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
