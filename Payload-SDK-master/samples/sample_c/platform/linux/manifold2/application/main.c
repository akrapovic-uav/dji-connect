...
#include <xport/TUMHFT_payload_xport.h>
    ...
    if (aircraftInfoBaseInfo.mountPosition == DJI_MOUNT_POSITION_EXTENSION_PORT &&
        ...
        if (aircraftInfoBaseInfo.djiAdapterType == DJI_SDK_ADAPTER_TYPE_XPORT) {
            if (DjiTest_XPortStartService() != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                USER_LOG_ERROR("PSDK X-PORT INIT ERROR. -tumhft");
            }
        }
#endif

#ifdef CONFIG_MODULE_TUMHFT_XPORT_ON
        USER_LOG_ERROR("STARTING X-PORT. -tumhft");
        if (aircraftInfoBaseInfo.djiAdapterType == DJI_SDK_ADAPTER_TYPE_XPORT) {
            if (DjiTest_XPortStartService_TUMHFT() != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                USER_LOG_ERROR("PSDK X-PORT INIT ERROR. -tumhft");
            }
        }
#endif
